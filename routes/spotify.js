...
async function upstashGet(key) {
  ...
  try {
    ...
    return resp.data && resp.data.result !== undefined
      ? resp.data.result
      : null;
  } catch (err) {
...
async function upstashSet(key, value) {
  ...
  const url = `${UPSTASH_URL}/set/${encodeURIComponent(
    key
  ...
  if (resp.status >= 400) {
    throw new Error(
      `Upstash set failed ${resp.status}: ${JSON.stringify(resp.data)}`
    );
  }
  return resp.data;
}
...
async function doRefreshTokenRequest(refreshToken) {
  ...
  if (resp.status >= 400) {
    const msg =
      resp.data && resp.data.error_description
...
async function fetchAccessToken() {
  ...
  try {
    ...
    console.warn(
      "Upstash read error (will attempt refresh):",
      (e && e.message) || e
    );
  }
  ...
  inFlightRefresh = (async () => {
    try {
      ...
      if (!refreshToken) {
        const e = new Error(
          "No refresh token available (seed Upstash or set SPOTIFY_REFRESH_TOKEN env)"
        );
        e.status = 500;
        throw e;
      }
...
function simplifyNowPlaying(payload) {
  ...
  const artistUrl =
    song.artists && song.artists[0] && song.artists[0].external_urls
      ? song.artists[0].external_urls.spotify
      : null;
  const albumUrl =
    song.album && song.album.external_urls
...
router.get("/now-playing", async function (req, res) {
  try {
    ...
    console.error(
      "Unexpected /spotify/now-playing error:",
