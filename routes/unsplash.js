...
const ACCESS_KEY =
  process.env.UNSPLASH_ACCESS_KEY || process.env.UNSPLASH_CLIENT_ID || null;
...
function cachedGetCollections() {
  if (cache.collections.data && Date.now() < cache.collections.expiresAt)
...
async function fetchFromUnsplash(path, params = {}) {
  if (!ACCESS_KEY) {
    const e = new Error(
      "Missing Unsplash access key (set UNSPLASH_ACCESS_KEY or UNSPLASH_CLIENT_ID)"
    );
    e.status = 500;
    throw e;
  }
...
router.get("/collections", async function (req, res) {
  try {
    ...
    if (!noCache) {
      ...
      if (cached)
        return res.json({
          meta: { source: "unsplash", cached: true },
          collections: cached,
        });
    }
    ...
    return res.json({
      meta: { source: "unsplash", cached: false },
      collections: data,
    });
  } catch (err) {
...
router.get("/collections/:id/photos", async function (req, res) {
  try {
    ...
    const data = await fetchFromUnsplash(
